...
function editPatient(index) {
    ...
    // initialize forms progressive reveal when using the Forms theme
    if (document.documentElement.getAttribute('data-theme') === 'forms') initFormSteps();
...
function cancelEdit() {
    ...
    // teardown forms stepper if present
    destroyFormSteps();
}  
function showForm() {
    document.getElementById("form").style.display = "block";
    ...
    // if forms theme is active, initialize single-question flow
    if (document.documentElement.getAttribute('data-theme') === 'forms') initFormSteps();
} 
...
function setTheme(theme) {
    ...
    if (logo) {
        ...
        else if (theme === 'forms') logo.textContent = '📋';
        else logo.textContent = '🩺';
    }
    const selector = document.getElementById('themeSelector');
    if (selector) selector.value = theme;

    // initialize or teardown forms-mode if form is open
    const form = document.getElementById('form');
    if (form && form.style.display !== 'none') {
        if (theme === 'forms') initFormSteps();
        else destroyFormSteps();
    }
...
if (patient.followup && patient.followup < patient.visitDate) {
    ...
    if (document.documentElement.getAttribute('data-theme') === 'forms') {
        destroyFormSteps();
        document.getElementById('form').style.display = 'none';
    }
...
// --- Forms-mode stepper: show one question at a time, require Next button to proceed
function initFormSteps() {
    const form = document.getElementById('form');
    if (!form) return;
    if (document.documentElement.getAttribute('data-theme') !== 'forms') return;

    const fields = Array.from(form.querySelectorAll('.form-grid > .field'));
    // include all fields (including checkbox) so 'Save contact' can appear as the last question
    const stepFields = fields;
    if (stepFields.length === 0) return;

    form.classList.add('forms-mode');
    document.documentElement.classList.add('forms-open');

    form._formStepFields = stepFields;

    // find first empty field to start on (better UX when editing)
    let startIndex = 0;
    for (let i = 0; i < stepFields.length; i++) {
        const input = stepFields[i].querySelector('input, select, textarea');
        if (input) {
            const filled = (input.type === 'checkbox') ? input.checked : (input.value && input.value.toString().trim() !== '');
            if (!filled) { startIndex = i; break; }
            startIndex = i + 1; // if all filled up to i, start after
        }
    }
    if (startIndex >= stepFields.length) startIndex = stepFields.length - 1;

    form._currentIndex = startIndex;

    stepFields.forEach((f, i) => {
        f.style.display = (i === startIndex) ? 'block' : 'none';
    });

    // nav with Back / Progress / Next
    let nav = form.querySelector('.forms-nav');
    if (!nav) {
    ...
    // forms-specific actions container (Close + Save) inserted below nav
    let formsActions = form.querySelector('.forms-actions');
    if (!formsActions) {
        formsActions = document.createElement('div');
        formsActions.className = 'forms-actions';

        const save = document.createElement('button');
        save.className = 'btn primary';
        save.type = 'button';
        save.id = 'formsSaveBtn';
        save.textContent = '💾 Save Patient';
        save.onclick = () => savePatient();
        ...
        formsActions.appendChild(save);
        formsActions.appendChild(closeBtn);
        form.appendChild(formsActions);
    }

    updateFormNav();

    // focus the starting input
    const startInput = stepFields[startIndex].querySelector('input, select, textarea');
    if (startInput) startInput.focus();
}

function updateFormNav() {
    const form = document.getElementById('form');
    if (!form || !form._formStepFields) return;
    const idx = form._currentIndex || 0;
    const n = form._formStepFields.length;
    const nav = form.querySelector('.forms-nav');
    if (!nav) return;
    const progress = nav.querySelector('.progress');
    const back = nav.querySelector('.btn');
    const next = nav.querySelector('.btn.primary');

    progress.textContent = `${idx + 1} / ${n}`;
    back.disabled = idx === 0;

    // On last step: hide the Next button and show the Save button below; otherwise show Next and hide Save
    const saveBtn = form.querySelector('.forms-actions .btn.primary');
    if (idx === n - 1) {
...
function nextFormStep() {
    ...
    const current = form._formStepFields[idx];
    const input = current.querySelector('input, select, textarea');

    // Basic required check if any input has 'required' attribute
    if (input && input.hasAttribute('required')) {
        const val = (input.type === 'checkbox') ? input.checked : input.value;
        if (!val || (typeof val === 'string' && val.trim() === '')) { input.focus(); showNotice('Please fill this field'); return; }
    }

    if (idx < n - 1) {
        current.style.display = 'none';
        form._currentIndex = idx + 1;
        const nextField = form._formStepFields[form._currentIndex];
        nextField.style.display = 'block';
        updateFormNav();
        const firstInput = nextField.querySelector('input, select, textarea');
...
function prevFormStep() {
    ...
    if (idx > 0) {
        ...
        current.style.display = 'none';
        form._currentIndex = idx - 1;
        const prevField = form._formStepFields[form._currentIndex];
        prevField.style.display = 'block';
        updateFormNav();
        const firstInput = prevField.querySelector('input, select, textarea');
        if (firstInput) firstInput.focus();
    }
}

function destroyFormSteps() {
    const form = document.getElementById('form');
    if (!form) return;
    form.classList.remove('forms-mode');
    document.documentElement.classList.remove('forms-open');

    // remove listeners
    if (form._formListeners) {
        form._formListeners.forEach(({el, handler}) => {
            el.removeEventListener('input', handler);
            el.removeEventListener('change', handler);
        });
        delete form._formListeners;
    }

    if (form._formStepFields) {
        form._formStepFields.forEach(f => { f.classList.remove('active'); f.style.display = ''; });
        delete form._formStepFields;
        delete form._visibleIndex;
        delete form._formStepIndex;
        delete form._currentIndex;
    }
    const nav = form.querySelector('.forms-nav');
    if (nav) nav.remove();
    const formsActions = form.querySelector('.forms-actions');
    if (formsActions) formsActions.remove();
}
...
function closeImportExportMenu(e) {
    const menu = document.getElementById('importExportMenu');
    const btn = document.getElementById('importExportBtn');
    if (!menu) return;
    if (e && (menu.contains(e.target) || (btn && btn.contains(e.target)))) return;
